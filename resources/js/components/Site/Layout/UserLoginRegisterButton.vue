<template>
    <div class="checkout-billing-area  mt-4">
        <div class="row">
            <div class="m-auto col-md-5 col-lg-5">
...
<script>
    export default {
        name: "UserLoginRegisterButton",
        props: ['nextPageUri'],
        data() {
            return {}
        }, methods: {
            goToLogin() {
                this.$router.push({name: 'login', query: {redirect: `/${this.nextPageUri}`}});
            },
            goToRegister() {
                this.$router.push({name: 'register', query: {redirect: `/${this.nextPageUri}`}});
            },
        }
    }
</script>
